...
#[Package('checkout')]
class FakeCustomerLoginRoute extends AbstractLoginRoute
{
    /**
     * @internal
     */
    ...
    #[Route(path: '/store-api/account/login/imitate-customer-login', name: 'store-api.account.login.imitate', methods: ['POST'])]
    public function login(RequestDataBag $data, SalesChannelContext $context): ContextTokenResponse
    {
        ...
        $token = $this->accountService->fakeCustomerLogin(
            $email,
            $context
        );
